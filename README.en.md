...
4. **Click-to-open without a transient tab.** On loopback GUI URLs, the
   ...
   elevation, virtual-desktop, and focus-stealing policy can still prevent
   foregrounding; in that case the session is nevertheless selected in the
   background. If no live GUI acknowledges promptly, or the protocol
   ...
   browser/Windows confirmation. Non-loopback custom `baseUrl` values keep
   the normal HTTP deep link for safety.
